; -*- lexical-binding: t -*-
;;; funcs.el --- sohunjug Layer packages File for Spacemacs
;;
...
(defun sohunjug/clearn-highlight ()
    (interactive)
...
(defun sohunjug/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun sohunjug/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun sohunjug/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun sohunjug/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun sohunjug/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "sohunjug")))

(defun sohunjug/save-my-layout ()
  (interactive)
  (persp-save-state-to-file (concat persp-save-dir "sohunjug")))
...
(defun sohunjug/indent-region(numSpaces)
  (progn
...
(defun sohunjug/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (sohunjug/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun sohunjug/untab-region (N)
  (interactive "p")
  (sohunjug/indent-region -4))

(defun sohunjug/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'sohunjug/tab-region)
  (local-set-key (kbd "<S-tab>") 'sohunjug/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'sohunjug/hack-tab-key)
...
(defun sohunjug/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(sohunjug//hotspots-sources))))

(defun sohunjug//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (sohunjug/run-current-file)))
...
(defun sohunjug/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun sohunjug/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (sohunjug/git-project-root)
      (counsel-git)
    (if (projectile-project-p)
        (projectile-find-file)
      (counsel-file-jump))))

(defun sohunjug/pomodoro-notification ()
  ...
  (if (spacemacs/system-is-mswindows)
    ...
    (progn (add-hook 'org-pomodoro-finished-hook '(lambda () (sohunjug/growl-notification "Pomodoro Finished" "Have a break!" t)))
           (add-hook 'org-pomodoro-short-break-finished-hook '(lambda () (sohunjug/growl-notification "Short Break" "Ready to Go?" t)))
             (add-hook 'org-pomodoro-long-break-finished-hook '(lambda () (sohunjug/growl-notification "Long Break" " Ready to Go?" t))))))
...
;; (defun sohunjug/growl-notification (title message &optional sticky)
...
(defun sohunjug/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (sohunjug/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun sohunjug/goto-match-paren (arg)
...
(defun sohunjug/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun sohunjug/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun sohunjug/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (sohunjug/retrieve-chrome-current-tab-url)))

(defun sohunjug/list-all-tabs ()
  (interactive)
  (ivy-read
...
(defun sohunjug/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun sohunjug/single-lines-only ()
...
(defun sohunjug/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun sohunjug/ash-term-hooks ()
...
(defun sohunjug/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun sohunjug/evil-quick-replace (beg end )
  (interactive "r")
...
(defun sohunjug/git-project-root ()
...
(defun sohunjug/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (sohunjug/git-project-root)
                default-directory))
...
(defun sohunjug/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (sohunjug/git-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun sohunjug/magit-visit-pull-request ()
...
(defun sohunjug/markdown-to-html ()
  (interactive)
...
(defun jumbo/github-browse-commit ()
...
(defun sohunjug/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun sohunjug/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun sohunjug/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
