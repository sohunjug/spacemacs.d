...
(defun sohunjug-misc/init-alert ()
  (use-package alert
    :config
...
(defun sohunjug-misc/post-init-popup ()
  (use-package popup
    :config
    (progn
...
(defun sohunjug-misc/init-lispyville ()
...
(defun sohunjug-misc/init-browse-at-remote ()
  (use-package browse-at-remote
    :defer t
    :init (spacemacs/set-leader-keys "gho" 'browse-at-remote)))

(defun sohunjug-misc/init-highlight-global ()
...
(defun sohunjug-misc/post-init-symbol-overlay ()
...
(defun sohunjug-misc/post-init-golden-ratio ()
...
(defun sohunjug-misc/post-init-ranger ()
...
(defun sohunjug-misc/init-helm-ag ()
  (use-package helm-ag
    :defer t
    :init
    (progn
...
(defun sohunjug-misc/post-init-gist ()
  (use-package gist
    :defer t
    :init
...
(defun sohunjug-misc/init-peep-dired ()
...
(defun sohunjug-misc/post-init-flyspell-correct ()
  (progn
    (with-eval-after-load 'flyspell
      (define-key flyspell-mode-map (kbd "C-;") 'flyspell-correct-previous-word-generic))
    (setq flyspell-correct-interface 'flyspell-correct-ivy)))

(defun sohunjug-misc/post-init-smartparens ()
...
(defun sohunjug-misc/init-tiny ()
  (use-package tiny
    :defer t
    :init
    (spacemacs/set-leader-keys "oe" 'tiny-expand)))

(defun sohunjug-misc/post-init-helm ()
...
(defun sohunjug-misc/init-helm-github-stars ()
  (use-package helm-github-stars
    :commands (helm-github-stars)
    :init
    (setq helm-github-stars-username "sohunjug")))




(defun sohunjug-misc/post-init-fcitx ()
  (fcitx-aggressive-setup))

(defun sohunjug-misc/post-init-command-log ()
...
(defun sohunjug-misc/init-litable ()
  (use-package litable
    :init
    :defer t))

(defun sohunjug-misc/init-osx-dictionary ()
...
(defun sohunjug-misc/init-4clojure ()
...
(defun sohunjug-misc/post-init-avy ()
  (progn
    (global-set-key (kbd "C-s-'") 'avy-goto-char-2)
    (global-set-key (kbd "M-'") 'avy-goto-char-2)))

(defun sohunjug-misc/post-init-ace-window ()
  (global-set-key (kbd "C-x C-o") #'ace-window))

(defun sohunjug-misc/init-discover-my-major ()
...
(defun sohunjug-misc/post-init-elfeed ()
  (use-package elfeed
    :init
    ...
    (progn
      ...
      (defun jumbo/elfeed-mark-all-as-read ()
        (interactive)
        (mark-whole-buffer)
        (elfeed-search-untag-all-unread))

      (define-key elfeed-search-mode-map (kbd "R") 'jumbo/elfeed-mark-all-as-read)
...
(defun sohunjug-misc/post-init-evil ()
  (progn
    ...
    (define-key evil-normal-state-map
      (kbd "Y") 'sohunjug/yank-to-end-of-line)
    ...
    (define-key evil-visual-state-map (kbd "C-r") 'sohunjug/evil-quick-replace)
    ...
    ;; (defadvice evil-insert-state (around sohunjug/holy-mode activate)
...
(defun sohunjug-misc/init-visual-regexp ()
  (use-package visual-regexp
    :commands (vr/replace vr/query-replace)))

(defun sohunjug-misc/init-visual-regexp-steroids ()
...
(defun sohunjug-misc/init-multiple-cursors ()
...
;;; Occasionally useful
    ...
    (setq mc/cmds-to-run-once
          '(
            counsel-M-x
            sohunjug/my-mc-mark-next-like-this))
...
(defun sohunjug-misc/post-init-persp-mode ()
  (setq persp-kill-foreign-buffer-behaviour 'kill)
  (setq persp-lighter nil)

  (defun sohunjug-kill-other-persp-buffers (&optional arg)
  ...
  (when (fboundp 'spacemacs|define-custom-layout)
    (spacemacs|define-custom-layout "@work"
      :binding "w"
      :body
      (find-file "~/Code/sohunjug/Github/HlMJ_js/assets/scripts/Login/LoginScene.ts")))
  (when (fboundp 'spacemacs|define-custom-layout)
    (spacemacs|define-custom-layout "@blog"
      :binding "b"
      :body
      (find-file "~/Documents/blog/config.toml")))
  )

(defun sohunjug-misc/init-rime ()
  (use-package rime
    :custom
...
(defun sohunjug-misc/post-init-pyim ()
  (progn
...
;; deprecated
(defun sohunjug-misc/post-init-chinese-wbim ()
  (progn
...
(defun sohunjug-misc/post-init-evil-escape ()
  (setq evil-escape-delay 0.2))

(defun sohunjug-misc/init-find-file-in-project ()
    ...
    (progn
      ...
      (defadvice find-file-in-project (before my-find-file-in-project activate compile)
        (when (ffip-current-full-filename-match-pattern-p "\\(HLMJ_js\\)")
          ...
          (setq-local ffip-project-root "~/Code/sohunjug/Github/HLMJ_js")
...
(defun sohunjug-misc/post-init-projectile ()
  (progn
  ...
  current project. Otherwise do `occur' in the current file."
    ...
    (spacemacs/set-leader-keys "pf" 'sohunjug/open-file-with-projectile-or-counsel-git)
    (spacemacs/set-leader-keys "pt" 'my-simple-todo)))



(defun sohunjug-misc/post-init-prodigy ()
  (progn
    (prodigy-define-tag
      :name 'jekyll
    ...
    ;;   (call-interactively 'sohunjug/browser-refresh--chrome-applescript))
...
(defun sohunjug-misc/init-moz-controller ()
...
(defun sohunjug-misc/init-ag ()
  (use-package ag
    :init))

(defun sohunjug-misc/post-init-erc ()
  (progn
    (add-hook 'erc-text-matched-hook 'my-erc-hook)
    (spaceline-toggle-erc-track-off)))

(defun sohunjug-misc/init-wrap-region ()
...
(defun sohunjug-misc/init-keyfreq ()
  (use-package keyfreq
    :init
    (progn
      (keyfreq-mode t)
...
(defun sohunjug-misc/post-init-swiper ()
...
(defun sohunjug-misc/post-init-magit ()
  (progn
    ...
    (setq magit-repository-directories '("~/Documents/Code/"))
    (setq magit-push-always-verify nil)

    (eval-after-load 'magit
      '(define-key magit-mode-map (kbd "C-c g")
         #'sohunjug/magit-visit-pull-request))

    (setq magit-process-popup-time 10)))

(defun sohunjug-misc/post-init-git-messenger ()
  (use-package git-messenger
    :defer t
    :config
    (progn
      (define-key git-messenger-map (kbd "f") 'sohunjug/github-browse-commit))))

(defun sohunjug-misc/post-init-markdown-mode ()
  (progn
