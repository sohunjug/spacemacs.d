...
;;
;; Author: sohunjug <guanghui8827@gmail.com>
;;
...
(defun sohunjug/directory-parent (directory)
...
;; Screenshot
(defun sohunjug//insert-org-or-md-img-link (prefix imagename)
  (if (equal (file-name-extension (buffer-file-name)) "org")
      (insert (format "[[%s%s]]" prefix imagename))
    (insert (format "![%s](%s%s)" imagename prefix imagename))))

(defun sohunjug/capture-screenshot (basename)
  ...
  (progn
    ...
    (sohunjug//insert-org-or-md-img-link "./" (concat basename ".png")))
  (insert "\n"))

(defun sohunjug/org-archive-done-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun sohunjug/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun sohunjug/skip-non-stuck-projects ()
...
(defun sohunjug/org-insert-src-block (src-code-type)
...
(defun sohunjug/org-summary-todo (n-done n-not-done)
  "Switch entry to DONE when all subentries are done, to TODO otherwise."
  (let (org-log-done org-log-states)    ; turn off logging
    (org-todo (if (= n-not-done 0) "DONE" "TODO"))))

(defun sohunjug/filter-by-tags ()
  (let ((head-tags (org-get-tags-at)))
    (member current-tag head-tags)))

(defun sohunjug/org-clock-sum-today-by-tags (timerange &optional tstart tend noinsert)
  (interactive "P")
    ...
    (while (setq file (pop files))
      ...
      (with-current-buffer org-agenda-buffer
        (dolist (current-tag include-tags)
          (org-clock-sum tstart tend 'sohunjug/filter-by-tags)
